...
func TestLinuxBatchRunsWaveAndTimesOutOnlySlowTarget(
	t *testing.T,
) {
	binary := buildService(t)
...
func TestLinuxCleanupSignalsEveryOwnedBatchTargetGroup(
	t *testing.T,
) {
	var groupSignals []int
	operations := linuxOperations{
	...
	err := runner.Cleanup(
		...
		task.ProcessLease{
			...
			TargetProcessGroups: []int{300, 301},
		},
		0,
	)
	if err != nil {
	...
	if !reflect.DeepEqual(groupSignals, []int{300, 301}) {
		t.Fatalf("group signals = %v", groupSignals)
	}
}
