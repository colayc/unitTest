        ...
        try {
          ...
          if (fault === "cancel" || fault === "timeout") {
            ...
            assert.equal(
              ...
              `coverage ${scenario} outcome ${run.outcome ?? "<none>"} reason ${run.reason ?? "<none>"} task=${coverageTask.outcome ?? "<none>"} taskError=${coverageTask.errorCode ?? "<none>"}:${coverageTask.errorMessage ?? "<none>"}`,
            );
