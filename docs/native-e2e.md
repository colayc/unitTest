...
Windows LLVM coverage required gate 另有更强的 OS 级边界：独立 Go preflight 先复用 production Windows discovery evidence 与 `coveragellvm.PinToolset/Verify` identity/digest verifier；它不输出工具路径，且必须在 firewall、Service start 和 native execution 前完成。unavailable 在 non-required 本机只产生精确单一 SKIP，在 required CI 则失败；verified 后才进入 guardian，guardian 失败仍是 FAIL。在真实 Service 启动前创建随机唯一、仅在测试时窗作用于隔离 runner 的 all-program outbound Block/Any Windows Firewall rule，因此 Service 以及 CMake、Ninja、clang-cl/linker、测试进程、llvm-profdata/llvm-cov 子进程树均无法出网。每次启动另生成 command-bound 256-bit nonce；唯一的 detached guardian 在 pre-audit 后核对 nonce/owner/self-PID closed state，再亲自创建 rule，分别复核 ActiveStore/PersistentStore rule identity、七项 policy 与各自六类 closed filters，并显式从 ActiveStore 验证严格为 Domain/Private/Public 且全部启用；create 前、ready 前、等待期与 cleanup transition 都持续复核 state，完整审计后才发布 ready，ready 后再无 create 路径。owner 退出或 explicit release 后，guardian 对 removal/query fault 持续有界重试到两个 store 连续为空，写入并复核 removed 后退出；Node 必须等待两项确认。CI `always()` cleanup 从固定 state root release 所有 guardian，并复用同一有界 group 清理/双 store 审计；root 以 `Get-ChildItem -Force` 拒绝任何普通文件、reparse 或未知 entry，state dir 使用含 `guardian.nonce` 的七 marker closed canonical schema。CleanupAll 还会独立枚举 live Windows PowerShell creator，并以原生 OS 查询严格匹配 executable、boundary script、Guard action、state root/direct-child state path、rule、owner 与 nonce；PID/removed marker 被伪造或 state directory 被替换都不能隐藏 creator。只有所有 matching creator 退出、closed state 成立且 Active/Persistent 连续为空才成功。权限、未知 guardian、创建、身份、审计或撤销失败均使 required gate 失败。
