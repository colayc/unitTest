...
Each client must send `handshake` first. Its token is supplied through a file owned by the current user. On Unix the file must not grant group or other permissions; on Windows it must have a protected DACL whose allow entries name only the current user. The service rejects symbolic links, bounds the file to 4 KiB, and must delete the same file it opened before startup can succeed. The protocol version is `1.0`.
