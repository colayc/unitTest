...
WFP 没有 PID-tree condition。支持的 coverage build 因此使用 closed LaunchPlan：planner 在启动 CMake/Ninja 前有界解析主 `CMakeLists.txt`、literal include/subdirectory、preset include/inheritance/cacheVariables 与 toolchain file，收集 `add_custom_command`、`add_custom_target`、`add_test` 与 `execute_process` 的每个 `COMMAND` executable，并校验 compiler launcher 及 global/directory/target `RULE_LAUNCH_*` property；`try_run`、CTest/Ninja/custom wrapper 与 configure-time graph generation/write 直接拒绝。fresh、静态 `add_executable` target 的确定性 artifact 可进入同一 plan，不依赖旧 File API state。所有入口必须精确映射到已注册 APP_ID。未解析的动态变量/生成表达式/preset/property、无法静态证明的 graph edge、未知 executable、shell/代启动 wrapper 或解析上限溢出都在 CreateProcess 前失败。完整 graph 的 OS identity 和 SHA-256 固定在 execution plan 中；processhost 重新验证后持有 no-write/no-delete handles 到 CMake 进程树退出，不能将任意动态 grandchildren 或 verify 后被替换/改写的 source graph 宣称为已覆盖。
