...
真实执行前先执行 verified-toolset preflight，再建立双层 offline boundary。preflight 是独立 Go 程序，直接复用 production `NewWindowsAdapters` discovery evidence 和 `coveragellvm.PinToolset/Verify` retained handle、Windows file identity、SHA-256 校验，输出 closed、无路径 availability JSON；不得以 PATH 或文件存在另建弱 verifier。它必须先于 firewall、Service start 与 native execution：unavailable 且非 required 时仅产生精确单一 SKIP，required 时 FAIL；verified 后才建立 guardian，guardian 失败仍 FAIL，后续 Service discovery 还要再次验证同一 version。现有 Node HTTP(S) guard 拦截测试 host API；随机唯一、仅在测试时窗作用于隔离 runner 的 Windows Firewall all-program outbound `Block/Any` rule 在 OS 层覆盖 Service、CMake/Ninja、compiler/linker、fixture test、`llvm-profdata` 与 `llvm-cov` 全子进程树。Rule 由唯一的长生命周期 guardian 进程负责：它先在 Active/Persistent 两个 store 做 absent pre-audit，再亲自创建；ActiveStore 与 PersistentStore 的 rule 必须分别获取并分别严格核对 Name/DisplayName/Enabled/Direction/Action/Profile/Group，application/address/port/service/interface/interfaceType filter 也必须逐 store 独立读取并为 closed Any。profile 查询必须显式使用 `Get-NetFirewallProfile -PolicyStore ActiveStore`，结果严格为全部 enabled 的 Domain/Private/Public。完整审计后才写 exclusive ready；ready 后 guardian 没有 create 分支，只等待 owner 退出或 explicit release，随后在 finally 中逐次捕获 removal/query fault，并在 bounded deadline 内重试到两个 store 连续为空，写 removed 后退出。正常 Node teardown 只有同时确认 removed 与 guardian 成功退出才恢复 HTTP guard；异常恢复先终止并确认唯一 creator 已退出，才允许 exact cleanup。

CI `always()` cleanup 从固定 state root release 所有 guardian，并循环 group removal、Active/Persistent audit 与 guardian convergence。每轮先执行全局删除和双 store 空审计，再独立枚举 OS live Windows PowerShell creator 并验证 guardian state。state root 用 `Get-ChildItem -Force` 做 closed enumeration，只允许名称 canonical 的普通 non-reparse guardian 目录；普通文件、symlink/junction、其他 reparse 与未知 entry 全部 fail-closed。每个目录只接受 `rule-name`、`owner.pid`、`guardian.nonce`、`guardian.pid`、`release`、`ready`、`removed` 七种 regular/non-reparse、大小有界、strict UTF-8、exact canonical marker。每个 guardian command line 都绑定独立 256-bit nonce；guardian 在 create 前、ready 前、等待期与 cleanup transition 持续验证 nonce、owner、自身 PID 与 closed state。CleanupAll 不以可写 marker 作为 live creator 索引，而通过原生只读 OS process query 严格匹配 Windows PowerShell executable、exact boundary script、Guard action、canonical state root/direct-child state path、rule、owner 和 nonce；任何 matching process 都先被请求 release 并在真实退出前保持 blocker。PID 替换、canonical removed 伪造或 state directory replacement 都不能绕过 live creator。只有所有 matching creator 退出、closed state/removal proof 成立以及稳定双 store 空审计全部可证明时 CleanupAll 才成功。extra/reparse/损坏/不一致状态、process command line、身份或权限不可审计、timeout 都 fail-closed。创建、审计或撤销失败均使 required CI 失败，绝不能回退为 Node-only evidence。

本机 preflight 未形成 verified `clang-cl`/`llvm-profdata`/`llvm-cov` capability 时，只允许在 native execution 前产生一个精确的 `SKIP: verified clang-cl coverage toolset is unavailable`，`pass` 为 0，且不生成 evidence file；该结果不是 native PASS。preflight verified 后，prepared CMake bundle 缺失属于执行门禁失败，不能再降级为工具链 SKIP。Windows CI 设置 `UNIT_TEST_IDE_NATIVE_REQUIRED_TOOLCHAINS=clang-cl`，把 toolset unavailable 提升为失败。成功证据要等 assertion/viewer、Service shutdown、fixture cleanup 和 offline boundary cleanup 全部结束后才可发布；exclusive 临时文件必须 flush、回读并验证 canonical bytes，最后一步才 atomic rename。任一 assertion、回读或 teardown fault 都必须保持 final report 不存在。`.native-e2e/artifacts/windows/coverage-execution-report.json` 的 strict JSON closed shape 只含 platform/architecture、compiler/driver/collector 版本、run/TestRun outcome、summary、三种 artifact digest/size 与 duration，不含 token、environment、ID、workspace/data/tool path 或 raw profile 名称；workflow 仅在 success 时上传，并继续以 `if-no-files-found: error` 约束缺报告的伪成功。
