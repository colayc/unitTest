...
CI `always()` cleanup 从固定 state root release 所有 guardian，并循环 group removal、Active/Persistent audit 与 guardian convergence。每轮先执行全局删除和双 store 空审计，再验证 guardian state；每个目录只接受 `rule-name`、`owner.pid`、`guardian.pid`、`release`、`ready`、`removed` 六种 regular/non-reparse、大小有界、strict UTF-8、exact canonical marker。PID 内容必须为严格正整数；存活进程还必须同时匹配 Windows PowerShell executable 以及含同一 rule、owner PID、state directory 的 CIM command line。可信 pre-ready guardian 收到 release 后仍是 blocker，canonical removed 也不能绕过 live process；只有 removed、进程退出和稳定双 store 空审计全部可证明时 CleanupAll 才成功。extra/reparse/损坏/不一致状态、身份或权限不可审计、timeout 都 fail-closed。创建、审计或撤销失败均使 required CI 失败，绝不能回退为 Node-only evidence。
