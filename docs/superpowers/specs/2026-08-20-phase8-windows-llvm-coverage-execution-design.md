...
真实执行前必须建立双层 offline boundary：现有 Node HTTP(S) guard 拦截测试 host API；随机唯一、仅在测试时窗作用于隔离 runner 的 Windows Firewall all-program outbound `Block/Any` rule 在 OS 层覆盖 Service、CMake/Ninja、compiler/linker、fixture test、`llvm-profdata` 与 `llvm-cov` 全子进程树。Rule 由唯一的长生命周期 guardian 进程负责：它先在 Active/Persistent 两个 store 做 absent pre-audit，再亲自创建并复核 direction/action/profile 以及 application/address/port/service/interface closed filters；profile 查询必须显式使用 `Get-NetFirewallProfile -PolicyStore ActiveStore`，结果严格为全部 enabled 的 Domain/Private/Public。完整审计后才写 exclusive ready；ready 后 guardian 没有 create 分支，只等待 owner 退出或 explicit release，随后在 finally 中逐次捕获 removal/query fault，并在 bounded deadline 内重试到两个 store 连续为空，写 removed 后退出。正常 Node teardown 只有同时确认 removed 与 guardian 成功退出才恢复 HTTP guard；异常恢复先终止并确认唯一 creator 已退出，才允许 exact cleanup。CI `always()` cleanup 从固定 state root release 所有 guardian，并循环 group removal、Active/Persistent audit 与 guardian convergence；任何未知 state、权限错误或 timeout 都失败。创建、审计或撤销失败均使 required CI 失败，绝不能回退为 Node-only evidence，且任何路径都不得留下专属 firewall state。
