...
- final fix wave（base `6a43ee5`）将成功 handshake 的 negotiated version 绑定到 `Connection`，普通 event 在 decode/delivery 前必须 exact-match；为所有 negotiated v1.4 既有 API response conversion 增加 post-request close-and-rethrow guard；Task/Artifact page validator 改为 canonical v1.4 message payload `$ref`；public barrel 导出 `TestSelectionModeV14`。focused security regressions 6 passed、0 failed，完整 Client test 更新为 86 passed、0 failed。
- final fix wave 使用 pinned Node 24.18.0、pnpm 11.4.0、Go 1.26.5 与 worktree-local cache 重跑 Client build、`check:protocol-generated`、`check:coverage-generated`、root `pnpm build` 和 `git diff --check`，全部通过；完整命令、RED PoC、legitimate controls 与一次环境 PATH 重试记录在 `.superpowers/sdd/2026-08-05-typescript-client-v1-4-coverage-plan/final-fix-report.md`。
