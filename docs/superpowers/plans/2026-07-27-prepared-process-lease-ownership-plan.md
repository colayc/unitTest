...
- ownership safety裁决：任何 non-circuit、non-recovery Close error都保留 active owner与 nonterminal Task，不再按 durable/lease-free分叉；若尚无 cause，首次 Close error必须在对外发布 unhealthy之前 claim `infrastructure_failed`，已有 cause不被覆盖；普通 timeout、cancel、termination result、process done、Publisher callback与其他 cleanup command不得重试，只有后续显式 Shutdown可以 retry；retry成功的 terminal mutation原子设置 `DeleteLease=true`，first-cause与最终 outcome保持不变。
...
- 任何 normal非 circuit Close failure都不得 terminalize，无论 durable lease是否存在；若尚无 cause，Close error处理时必须先 `resolve(OutcomeInfrastructureFailed)`并按结果设置 `failPendingStep`，随后才能 `healthy.Store(false)`；
- cause/health ordering由同 package确定性 barrier测试覆盖；外部行为测试继续断言最终 outcome与 Step状态，不再依赖 watcher scheduling证明内部 ordering；
- 已有 cancel/timeout/interrupted等 cause不得被 Close failure或后续 Shutdown覆盖；显式 Shutdown retry Close成功后才按固定的 first-cause与最终 outcome terminalize，并用 `DeleteLease=true` 原子删除 durable lease；
- 删除 `finishAfterCloseFailure` 终态化路径及直接测试；公开 Manager 状态机必须覆盖 prepare error + durable lease、retry成功、retry再次失败和 restart recovery。
...
```powershell
& $go test ./apps/test-service/internal/task -run '^(TestManagerPersistsPreparedLeaseBeforeStartAndRefreshesItAfterStart|TestManagerPersistsPreparedLeaseBeforeHandlingClaimedCause|TestManagerPrepareErrorWithProcessPersistsLeaseBeforeCleanup|TestManagerPrepareErrorCloseFailureRetainsCleanupOwnershipUntilShutdown|TestManagerPrepareErrorCloseRetryFailureKeepsTaskAndLeaseNonterminal|TestManagerPublisherFailureCloseErrorHandsOffQueuedPreparedLease|TestManagerPreparedLease)' -count=1 -v
& $go test ./apps/test-service/internal/taskstore -run '^TestRecoverInterruptedFinishesAllActiveTasksAndDeletesLeases$' -count=1 -v
& $go test ./apps/test-service/internal/runtime -run '^TestOpenCleansQueuedPreparedLeaseBeforeInterruptedRecovery$' -count=1 -v
```
...
如果 first-cause、normal Close或旧 Publisher tests失败，修复 production ordering/state，不得删除断言或改成 sleep。durable与 lease-free normal Close回归都必须断言：首次 Close error后 Task仍 nonterminal且 owner仍 active；无先行 cause时先固定 infrastructure failure，有 cancel/timeout/interrupted时保留原 cause；普通 command不 retry；显式 Shutdown触发 Close retry，retry成功后才出现固定 outcome的 terminal state，durable lease通过 `DeleteLease=true` 原子删除。
...
既有 normal Close failure回归仍覆盖 durable与 lease-free两类，但两者统一保留 active owner和 nonterminal Task，terminal visibility都延迟到显式 Shutdown Close retry成功；first-cause与最终 outcome不变，durable路径额外断言 lease删除与 restart可见性。
...
```powershell
& $go test ./apps/test-service/internal/task -run '^(TestManagerPersistsPreparedLease|TestManagerPrepareErrorWithProcess|TestManagerPrepareErrorClose|TestManagerPublisherFailureCloseErrorHandsOffQueuedPreparedLease|TestManagerPreparedLease)' -count=100
& $go test ./apps/test-service/internal/task -run '^TestManagerPublisherFailure' -count=100
& $go test ./apps/test-service/internal/task -run '^TestManagerTimeoutCommandDoesNotRetryFailedCloseBeforeShutdown$' -count=100
...
& $go test -race ./apps/test-service/internal/task -run '^(TestManagerPersistsPreparedLease|TestManagerPrepareErrorWithProcess|TestManagerPrepareErrorClose|TestManagerPublisherFailure|TestManagerPreparedLease|TestManagerCancellation|TestManagerCloseFailure)' -count=1
```
...
Task 1: ownership-safety ruling approved (all normal Close errors retain active owner/nonterminal Task; terminal visibility is deferred until explicit Close retry succeeds)
...
Task 3: ownership-safety ruling approved (all normal Close errors retain active owner/nonterminal Task; terminal visibility is deferred until explicit Close retry succeeds)
