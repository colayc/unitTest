...
当前进度：Coverage Report Extension 的 command/controller/viewer 与 Go Service production `coverage/runs/start`、report persistence、共享 execution coordinator 已接通。Windows required-PASS CI 现在运行真实 `clang-cl/llvm-profdata/llvm-cov` Named Pipe + Protocol v1.4 smoke：关联 TestRun 必须为 `failed`、CoverageRun 必须为 `available`，Coverage JSON/JUnit/HTML 三种 artifact 必须完成 chunk、size、SHA-256、shared decoder、strict XML tokenizer 与无网络 viewer 校验。smoke 首先用独立 Go preflight 复用 production discovery evidence 与 LLVM retained-identity/digest verifier；unavailable non-required 在任何 firewall/Service/native side effect 前精确 SKIP，required 则失败，verified 才进入 guardian。执行期间由 ActiveStore 可审计、仅在测试时窗作用于隔离 runner 的 Windows Firewall all-program outbound Block/Any rule 覆盖整个 native 子进程树；每次启动生成 command/state 双绑定的 256-bit nonce，唯一 guardian 持续核对自身 PID/nonce closed state，完成 pre-audit、创建、Active/Persistent 各自 rule/policy/六类 filter 与 ActiveStore exact-profile 审计后才 ready，ready 后只等待 owner/release，再以 bounded retry 清理到稳定为空并退出。Node teardown 等 removed+exit；CI always cleanup 对固定 state root 与七 marker state 都做 closed enumeration，并独立枚举 live Windows PowerShell creator，以原生进程查询绑定 exact executable/script/Guard action/root/state/rule/owner/nonce，release 后等待所有 matching process 退出，再复用 group 收敛审计。普通 root file/reparse/unknown entry、dead-PID substitution、伪造 removed 或被替换的 state directory 都不能使 cleanup 提前成功。查询权限、进程命令行、guardian 身份/状态或建立/清理 OS 边界不能被证明时必须 fail-closed。strict JSON evidence 只在所有 assertion、Service/fixture/offline teardown 与临时文件回读成功后原子发布。未准备 verified toolset 的本机运行只能明确 SKIP，不能记为 PASS；Windows Hosted job 缺 toolset、offline boundary 或 report 会失败，失败 smoke 不上传 coverage evidence。Linux job 继续运行真实 Unix Socket Service smoke，但不运行或上传 native coverage report；Linux GCC/Clang coverage execution 尚未实现，cross-compile 不构成 native PASS。
