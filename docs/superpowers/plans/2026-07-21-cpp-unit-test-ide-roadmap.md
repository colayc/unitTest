...
受信任 producer 已定义为 `.github/workflows/release-inputs.yml`：它固定 Code-OSS commit `b1c0a14de1414fcdaa400695b4db1c0799bc3124`，在 GitHub-hosted `windows-2022`（VS2022/C++/Spectre fail-closed）和 `ubuntu-24.04` 上产生 1 天保留的固定 logical names `code-oss-windows-x64`、`code-oss-linux-x64`、`appimagetool-linux-x64` 以及 provenance；transport names are suffixed by the run attempt。run ID alone is not a complete artifact identity，run attempt 必须端到端绑定；runtime/tool downloads use immutable artifact IDs and upload digests，同时继续执行 content/inventory checks。producer re-run requires a new foundation dispatch，且 package jobs revalidate the attempt before and after download。一次 `release_version=0.1.0`、`release_signing_required=0` 的无签名资格验证只能作为短期测试证据；它不发布 GitHub Release，也不构成生产发布。本地 runtime 或 `release-inputs/code-oss.exe` 不能成为 producer 输入。
