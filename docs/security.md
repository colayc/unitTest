...
发布输入 producer 是 `.github/workflows/release-inputs.yml`，只允许 `master` 上的 input-free `workflow_dispatch`。它在 GitHub-hosted `windows-2022`（Visual Studio 2022/C++/Spectre preflight fail-closed）与 `ubuntu-24.04` 上生成固定名 `code-oss-windows-x64`、`code-oss-linux-x64`、`appimagetool-linux-x64` 和闭集 `release-input-provenance`；所有这些 artifact 的 retention 都是 1 天。producer 禁止读取本地 Code-OSS runtime 或 `release-inputs/code-oss.exe`，也禁止使用管理员/WFP self-hosted runner。

foundation 的 consumer 采用两个独立阶段：先通过 GitHub Actions run API 验证 producer repository、workflow path、event、master branch、head SHA 与 success conclusion；只有通过后才下载并验证 provenance。随后 package job 在 post-transport 阶段重新验证完整 runtime、launcher 摘要、Linux mode inventory 与 appimagetool，staging 与 package manifest 再绑定最终入包的文件集合和摘要。也就是 run API → provenance → post-transport → package-manifest/staging 的闭合信任链，package job 不得直接消费手工 dispatch 的 run ID 或摘要。
