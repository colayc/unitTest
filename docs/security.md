...
发布输入 producer 是 `.github/workflows/release-inputs.yml`，只允许 `master` 上的 input-free `workflow_dispatch`。它在 GitHub-hosted `windows-2022`（Visual Studio 2022/C++/Spectre preflight fail-closed）与 `ubuntu-24.04` 上生成固定的 logical artifact names：`code-oss-windows-x64`、`code-oss-linux-x64`、`appimagetool-linux-x64` 和闭集 `release-input-provenance`；transport names are suffixed by the run attempt，所有这些 artifact 的 retention 都是 1 天。producer 禁止读取本地 Code-OSS runtime 或 `release-inputs/code-oss.exe`，也禁止使用管理员/WFP self-hosted runner。

foundation 的 consumer 采用两个独立阶段：先通过 GitHub Actions run API 验证 producer repository、workflow path、event、master branch、head SHA 与 success conclusion；run ID alone is not a complete artifact identity，必须将 run attempt 端到端绑定。只有通过后才以 immutable artifact IDs 下载 provenance 与 runtime/tool artifacts，并以 upload digests 验证；既有 content 与 inventory checks 仍然执行。随后 package jobs revalidate the attempt before and after download，并在 post-transport 阶段重新验证完整 runtime、launcher 摘要、Linux mode inventory 与 appimagetool，staging 与 package manifest 再绑定最终入包的文件集合和摘要。也就是 run API → provenance → post-transport → package-manifest/staging 的闭合信任链，package job 不得直接消费手工 dispatch 的 run ID 或摘要。producer re-run requires a new foundation dispatch，旧 dispatch 不得绑定新的 attempt。

发布 workflow 从受信任 run 下载完整 Code-OSS runtime 目录制品，固定 Windows 根 launcher `Code - OSS.exe` 或 Linux `runtime/code-oss` 的 SHA-256，再把完整目录交给 staging；不会把单个可执行文件当成完整 runtime。Linux 固定名制品根只允许 `runtime/` 与一个闭集 `code-oss-runtime-mode.json`：该清单绑定 Linux/x64、launcher 摘要以及完整文件路径/大小/摘要/可执行位集合。GitHub artifact 下载丢失 Unix mode 后，消费者必须先验证受信任 run-id、根 launcher 摘要和清单闭集，再拒绝缺失、额外、别名、链接、特殊或摘要漂移的 entry，按清单统一恢复 `0755`/`0644` 并复核真实 Code-OSS runtime；禁止只给 launcher 补执行位。staging 与 release manifest 保留全部 runtime 依赖，安装后的固定 launcher 分别是 `app/code-oss-runtime/Code - OSS.exe` 与 `app/code-oss-runtime/code-oss`。

首次 `release_version=0.1.0` / `release_signing_required=0` 资格 run 的无签名包仅是 1 天内可检查的测试证据：不创建 GitHub Release，不构成生产发布。它不能代替正式 Windows 签名，也不能代替最终第三方 license/legal 审查；这两项仍是 Phase 8 未完成的工作。

Windows PowerShell 与 Linux Bash clean-machine smoke 都先下载 package job 产生的真实 package artifact，按 workflow 传递的 SHA-256 验证 MSIX/AppImage 及其 embedded release manifest，再从两个真实产生的 baseline/release package 提取精确 payload，并仅在一次性 root 中验证首次安装、`--version` launch handshake 和升级。Linux wrapper 对 target/baseline AppImage 先完成 byte digest 比对，再统一恢复 execute bit，之后 verifier 才能执行任一镜像。Target payload 在安装前及复制后都必须通过 manifest 验证；随后 smoke 只把已安装 target version 的平台固定 launcher 改写成确定性无效字节，保持同一 `app/code-oss-runtime/resources/app/product.json` 等 manifest-bound sibling bytes 不变，不修改下载 package 或 extracted source payload，并调用该 installed launcher。只有观察到启动失败才允许回滚。恢复到 manifest-verified baseline 后再次真实启动 baseline launcher，再验证重复回滚、卸载、user-data 保留和 package residue 清零。Windows smoke 为无扩展名 PE 创建短生命周期 hardlink alias 以调用同一 installed file identity，并在启动后立即删除。CI evidence 固定 target package filename/version/digests、`rollbackVersion` 和 source commit，并分别记录 `upgradeLaunch: failed-as-expected`、`rollback: pass`、`rollbackLaunch: pass`；不包含 token、environment、用户名、runner path 或 workspace 内容。
