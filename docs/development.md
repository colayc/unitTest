...
进入真实 Service 前，Windows smoke 同时安装 Node HTTP(S) guard 和一个随机唯一、仅在测试时窗作用于隔离 runner 的 Windows Firewall all-program outbound `Block/Any` rule。该 OS rule 不是 Node monkeypatch：它覆盖 Service 及其 CMake、Ninja、compiler/linker、测试进程、`llvm-profdata`、`llvm-cov` 整棵子进程树。唯一的长生命周期 guardian PowerShell 进程先确认 rule 在 ActiveStore/PersistentStore 均不存在，再亲自创建；随后分别读取两个 store 的 rule 对象并分别复核 Name/DisplayName/Enabled/Direction/Action/Profile/Group 与 application/address/port/service/interface/interfaceType closed filters，不能用一个 store 的对象替另一个 store 背书。Firewall profile 必须用 `Get-NetFirewallProfile -PolicyStore ActiveStore` 读取，且集合严格为已启用的 Domain/Private/Public。完整审计后 guardian 才写 exclusive ready marker；此后该进程没有任何 create 路径，只等待 owner 退出或 Node 写入 release。finally 对每次 removal/query 异常分别捕获并在有界期限内重试，连续确认两个 store 为空后才写 removed marker 并退出。正常 Node teardown 必须同时等到 removed 和 guardian 成功退出才恢复 HTTP guard；异常/超时恢复必须先终止并确认唯一 creator 已退出，才能启动独立 exact cleanup，未证明 OS rule 消失时继续 fail-closed。

Windows CI `always()` cleanup 先在每轮执行专属 group 删除与 Active/Persistent 空状态审计，再检查固定 state root。每个 guardian 目录只允许 `rule-name`、`owner.pid`、`guardian.pid`、`release`、`ready`、`removed` 六种 regular、non-reparse、大小有界、strict UTF-8 且内容 canonical 的 marker；extra leaf、非法 PID、内容不一致或 reparse 一律不能作为清理成功。对可信的 pre-ready guardian，CleanupAll 写入 canonical release，并以 Windows PowerShell executable 和 CIM command line（rule、owner、state directory）确认 PID 身份；即使已经出现 removed leaf，只要对应进程仍存活就继续等待。只有 canonical removed、对应 guardian 已退出以及连续双 store 空审计同时成立才能收敛；权限、身份、状态或超时不可证明时均 fail-closed，不能降级为 Node-only PASS。
