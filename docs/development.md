...
进入真实 Service 前，Windows smoke 同时安装 Node HTTP(S) guard 和一个随机唯一、仅在测试时窗作用于隔离 runner 的 Windows Firewall all-program outbound `Block/Any` rule。该 OS rule 不是 Node monkeypatch：它覆盖 Service 及其 CMake、Ninja、compiler/linker、测试进程、`llvm-profdata`、`llvm-cov` 整棵子进程树。Node 为每次启动生成 256-bit nonce，并同时绑定 guardian command line 与 durable state。唯一的长生命周期 guardian PowerShell 进程先确认 rule 在 ActiveStore/PersistentStore 均不存在，再核对 command-bound nonce、owner 与自身 PID 的 closed marker state 后亲自创建；随后分别读取两个 store 的 rule 对象并分别复核 Name/DisplayName/Enabled/Direction/Action/Profile/Group 与 application/address/port/service/interface/interfaceType closed filters，不能用一个 store 的对象替另一个 store 背书。Firewall profile 必须用 `Get-NetFirewallProfile -PolicyStore ActiveStore` 读取，且集合严格为已启用的 Domain/Private/Public。guardian 在 create 前、ready 前、等待循环和 cleanup transition 都重新核对自身 PID/nonce 与完整 closed state；完整 firewall 审计后才写 exclusive ready marker，此后该进程没有任何 create 路径，只等待 owner 退出或 Node 写入 release。finally 对每次 removal/query 异常分别捕获并在有界期限内重试，连续确认两个 store 为空且 state 仍 canonical 后才写并复核 removed marker，再退出。正常 Node teardown 必须同时等到 removed 和 guardian 成功退出才恢复 HTTP guard；异常/超时恢复必须先终止并确认唯一 creator 已退出，才能启动独立 exact cleanup，未证明 OS rule 消失时继续 fail-closed。

Windows CI `always()` cleanup 先在每轮执行专属 group 删除与 Active/Persistent 空状态审计，再检查固定 state root。root 使用 `Get-ChildItem -Force` 做闭集枚举，只允许名称 canonical 的普通、non-reparse guardian 目录；普通文件、symlink/junction、其他 reparse 或未知 leaf/目录全部 fail-closed。每个 guardian 目录只允许 `rule-name`、`owner.pid`、`guardian.nonce`、`guardian.pid`、`release`、`ready`、`removed` 七种 regular、non-reparse、大小有界、strict UTF-8 且内容 canonical 的 marker；extra leaf、非法 nonce/PID、内容不一致或 reparse 一律不能作为清理成功。CleanupAll 不依赖可写 PID/removed marker 寻找 creator：每轮独立枚举 OS 中的 Windows PowerShell 进程，并用原生只读进程查询严格绑定 executable、boundary script、`Guard` action、canonical state root/direct-child state directory、rule、owner 与 256-bit nonce。每个 matching guardian 都先收到 canonical release（目录仍安全时）并持续作为 blocker，直到真实退出；即使 marker 被替换为 dead PID、removed 被伪造或整个 state directory 被替换，也不能提前成功。只有所有 matching creator 已退出、root/state 闭集成立、canonical removed proof（仍存在的 state）成立，以及连续双 store 空审计同时成立才能收敛；权限、进程命令行、身份、状态或超时不可证明时均 fail-closed，不能降级为 Node-only PASS。
