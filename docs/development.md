...
进入真实 Service 前，Windows smoke 同时安装 Node HTTP(S) guard 并等待 native WFP guardian 完成 authenticated `Hello`/`Ready`。WFP filters 只作用于 closed LaunchPlan 中精确注册的 APP_ID，且只阻断 non-loopback outbound；它不声称存在 Windows PID-tree filter。为防止 CMake/Ninja custom command 以未注册 executable 绕过 APP_ID，Service planner 在 processhost/CreateProcess 前有界解析 statically reachable 的 CMake graph；literal nested/generated include 可被证明，任何动态/缺失/越界 graph edge 或不在 allowlist 的 `COMMAND` 都 fail closed。processhost 随后必须先注册整个声明，任一 registration 失败都不能创建 Job 或目标进程。
