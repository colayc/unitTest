...
export function validateArchiveEntries(entries, input) {
  ...
  for (const entry of entries) {
    ...
    if ((normalized !== input.sourceDirectory && !entry.path.startsWith(root)) || seen.has(normalized)) throw failure("FRAMEWORK_ARCHIVE_UNSAFE", "archive path escapes or duplicates its source root");
...
async function auditTree(root, input) {
  const source = join(root, input.sourceDirectory); const metadata = await lstat(source);
  ...
  await walk(source); const marker = input.id === "cpputest" ? "CMakeLists.txt" : input.id === "unity" ? "src/unity.c" : "lib/cmock.rb"; const markerStat = await lstat(join(source, marker));
  if (!markerStat.isFile() || markerStat.isSymbolicLink()) throw failure("FRAMEWORK_ARCHIVE_UNSAFE", `source marker is missing: ${input.id}`);
  const license = join(source, input.license.path); const licenseStat = await lstat(license); if (!licenseStat.isFile() || licenseStat.isSymbolicLink() || await sha256File(license) !== input.license.sha256) throw failure("FRAMEWORK_LICENSE_MISMATCH", `license identity mismatch: ${input.id}`);
  if (await directoryDigest(source) !== input.treeSha256) throw failure("FRAMEWORK_ARCHIVE_UNSAFE", `source tree identity mismatch: ${input.id}`);
}
...
export async function prepareFrameworkBundle(options = {}) {
  ...
  await mkdir(cacheRoot, { recursive: true, mode: 0o700 }); await mkdir(dirname(target), { recursive: true, mode: 0o700 }); const staging = join(dirname(target), randomName(".framework-bundle-")); await mkdir(staging, { mode: 0o700 });
  try { for (const input of manifest.frameworks) { const archive = await ensureArchive(cacheRoot, input, ops.download ?? defaultDownload); const entries = ops.inspectArchive ? await ops.inspectArchive(input, archive) : await inspectArchive(archive); validateArchiveEntries(entries, input); if (ops.extractArchive) await ops.extractArchive(input, staging, archive); else await extractArchive(archive, staging); }
    const actualRoots = (await readdir(staging, { withFileTypes: true })).filter((entry) => entry.isDirectory()).map((entry) => entry.name).sort(); const expectedRoots = manifest.frameworks.map((item) => item.sourceDirectory).sort(); if (JSON.stringify(actualRoots) !== JSON.stringify(expectedRoots)) throw failure("FRAMEWORK_ARCHIVE_UNSAFE", "extraction roots do not match manifest");
