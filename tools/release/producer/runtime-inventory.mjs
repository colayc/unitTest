...
async function assertCodeOssIdentity(files, platform, expectedLauncherSha256) {
  const byPath = new Map(files.map((file) => [file.path, file]));
  const launcher = byPath.get(layouts[platform]);
  const product = byPath.get("resources/app/product.json");
  if (launcher === undefined || product === undefined || !byPath.has("resources/app/package.json")) {
  ...
  if (launcherDigest !== expectedLauncherSha256) {
...
export async function createRuntimeInventory(request = {}) {
  ...
  await assertCodeOssIdentity(tree.files, platform, expectedLauncherSha256);
  ...
  for (const file of tree.files) {
    ...
    if (modeRecord && (modeRecord.size !== file.size || modeRecord.sha256 !== sha256)) {
    ...
    records.push({ path: file.path, size: file.size, sha256, executable: modeRecord?.executable ?? false });
