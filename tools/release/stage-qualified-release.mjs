...
async function main(args) { const input = {}; for (let i = 0; i < args.length; i += 1) { const flag = args[i]; const key = cliMap.get(flag); if (!key) throw stagingFailure(`unknown argument: ${flag}`); if (Object.hasOwn(input, key)) throw stagingFailure(`duplicate argument: ${flag}`); if (i + 1 >= args.length || args[i + 1].startsWith("--")) throw stagingFailure(`missing value for ${flag}`); input[key] = args[++i]; } const result = await stageQualifiedRelease(input); process.stdout.write(`${JSON.stringify(result)}\n`); }
